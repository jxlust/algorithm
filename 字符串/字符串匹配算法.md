...
算法的思想是：假设现在文本串 S 匹配到 i 位置，模式串 P 匹配到 j 位置如果 j = -1(标记)，或者当前字符匹配成功（即 S[i] == P[j]），都令 i++，j++，继续匹配下一个字符；
如果 j != -1，且当前字符匹配失败（即 S[i] != P[j]），则令 i 不变，j = next[j]。意味失配时，模式串 P 相对于文本串 S 向右移动了 j - next [j] 位。换言之，当匹配失败时，模式串向右移动的位数为：失配字符所在位置 - 失配字符对应的 next 值。即移动的实际位数为：j - next[j]，且此值大于等于 1。

Next 数组的值含义是：代表失配前的字符串中，有多大长度的相同的前缀后缀。比如 Next[j] = k；表示 j 之前的字符串中有最大长度为 k 的相同前缀后缀。
此也意味着在某个字符失配时，该字符对应的 next 值会告诉你下一步匹配中，模式串应该跳到 j-Next[j]这个位置上。所以重点在于求 Next[]。

#### 其他理解

比如模式串：ABCABZ，文本串 ABCABXYZ，失去匹配时，是在 X 的位置失配，则探讨 ABCAB 这个串怎么进行移动。计算出他的公共的最长前缀和最长后缀的长度，就是失配字符的 next 值。比如 ABCAB 这个串的公共最长前后缀为 AB,最长长度为 2。这个时候文本串需要把 X 位置保持不变，然后模式串 j->2,即到了 C 这个字符和 X 字符对齐做比对。

```txt
S:   A  B  C  A  B  X  Y  Z
P:            A  B  C  A  B  Z  最长前缀字符和最长后缀字符去对齐
```
可以理解出来，j的下一个位置就时存入到next[i]里面，只需要通过模式串的next数组
```txt
P:       A  B  C  A  B  Z
i:       0  1  2  3  4  5  
next:    0  0  0  0  1  2  
```
i下标表示前i个字符，特殊的是next[0] = 0,next[1] = 0,因为一个字符没有公共前后缀。next[i]注意一点，前提条件就是当前位置i失去匹配后,移动调整的位置。
...
所以，我们将 P[0...i - 1] 的最长公共前后缀的长度作为 next[i] 的值，就得到了 next 数组。
